  ...
  export default function IndexPage({ data }) {
    return (
      <Layout>
        ...
        <ul>
          {
            data.allDrupalBlogPost.edges.map(({ node }) => (
...
Woo hoo! If you see some posts like below, you know that you are officially done with Part One!

<!-- TODO: Add Part One Screenshot -->
...
Once created, we'll want to give life to this component. Paste the following content into your version of `src/templates/blog-post.js`:
...
<details>
  ...
  exports.createPages = async function createPages({ actions, graphql }) {
    ...
    result.allDrupalBlogPost.edges.forEach(({ node }) => {
      createPage({
        component: blogPostTemplate,
...
You will be successful if you can (still!) display your posts, but now clicking one of the posts will link to one of your posts! If you can link to a post, you can move on to the next step! At this point, we want to augment our BlogPost template so that we can actually _display_ a post in more detail!
...
We will want to use this `$slug` variable, and query for a blog post by its slug. A convention in GraphQL is that if _all_ resources are available as `allResource` then `resource` is the resolver we can use to query a single resource, for example:

```graphql
export const blogPostQuery = graphql`
  query GetBlogPostBySlug($slug: String) {
    someResource({ slug: { eq: $slug }}) {
      title
